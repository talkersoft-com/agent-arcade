...
// The address carries the EDITION as well as the dev/prod split. The daemon is
// single-tenant — one backend, one token, one socket — so the two editions run
// their own rather than fighting over one. Binding the same address is what makes
// the ownership watchdog force a daemon to step down (go/hub.go), which must
// never happen between a free and a paid instance.
//
// This MUST agree with go/transport.go's addrSuffix(). Two answers here is how a
// client ends up talking to one daemon while another serves someone else.
function addrSuffix() {
  const ed = require("./edition");
  let part = "";
  try {
    part = ed.isCloud() ? ".cloud" : ".local";
  } catch { part = ""; } // edition not resolved (a standalone tool) → legacy name
  return part + (DEV ? ".dev" : "");
}

function localAddr() {
  const suffix = addrSuffix();
  if (process.platform === "win32") return `\\\\.\\pipe\\agent-arcade-dictation${suffix}`;
  return path.join(os.homedir(), ".hv", `dictation${suffix}.sock`);
}

// A daemon may legitimately have NO backend: free-plan dictation is opt-in and
// needs a speech server installed separately. It starts anyway, reports itself
// unhealthy with a reason, and refuses jobs out loud — which beats the app
// capturing audio for a daemon that was never allowed to exist.
function spawnDaemon(bin, apiUrl) {
  if (!bin || !fs.existsSync(bin)) return false;
  try {
    const ed = require("./edition");
    let edition = "";
    try { edition = ed.current(); } catch {}
    spawn(bin, ["--daemon"], {
      env: {
        ...process.env,
        DICTATION_API_URL: apiUrl || "",
        DICTATION_PROVIDER: apiUrl ? "http" : "none",
        ...(edition ? { DICTATION_EDITION: edition } : {}),
      },
      detached: true,
      stdio: "ignore",
    }).unref();
...
function connectDictation(opts) {
  ...
  // token() yields the current Talkersoft ID access token (or "") — sent in
  // hello so the first dictation after a reconnect is already authenticated.
  ...
  function connect() {
    ...
    sock.on("connect", () => {
      sock.write(JSON.stringify({ type: "hello", client, app_version: appVersion || "", protocol: 1, token: token() || "" }) + "\n");
    });
    sock.on("data", (chunk) => {
      buf += chunk;
      let nl;
  ...
  // setToken pushes a live token update to the daemon (login / refresh / logout).
  // Safe to call anytime; a no-op false return just means "not connected yet",
  // and the next hello will carry the current token() anyway.
  ev.setToken = (t) => send({ type: "token", token: t || "" });
