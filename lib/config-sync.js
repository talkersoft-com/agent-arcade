...
async function syncOnLogin({ token, deviceId, readDoc, device, avatarsDir, log = () => {}, onMigrating = () => {} }) {
  ...
  if (!status.migrated) {
    // The backend flag said this machine has never imported — the ONE moment the
    // YAML travels. Say so before it moves.
    onMigrating({
      agents: localAgents.length,
      groups: Array.isArray(doc.groups) ? doc.groups.length : 0,
      commands: Array.isArray(doc.commands) ? doc.commands.length : 0,
    });
    try {
