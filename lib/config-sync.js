...
async function syncOnLogin({ token, deviceId, readDoc, writeDoc, avatarsDir, log = () => {} }) {
  ...
  // Don't write systems back into the YAML — the concept is retired, and leaving
  // stale entries there would keep the Arcade's (now meaningless) machine filter
  // alive on this device.
  delete doc.systems;
  writeDoc(doc);
