...
function create({ cachePath, token, deviceId, device, localFallback = () => [], log = () => {}, onChange = () => {}, fetchImpl = fetch }) {
  ...
  let workspaceName = ""; // which arrangement this device is on (scoped reads name it)
  ...
  function readCache() {
    try {
      ...
      if (typeof raw.workspace_name === "string") workspaceName = raw.workspace_name;
      return out;
    } catch { return null; }
  }
  function writeCache(next) {
    try {
      ...
      fs.writeFileSync(cachePath, JSON.stringify({ ...next, workspace_name: workspaceName }));
  ...
  function adopt(cfg) {
    ...
    if (typeof cfg.workspace_name === "string") workspaceName = cfg.workspace_name;
    const next = {};
  ...
  return {
    ...
    status() {
      return { kind: "cloud", ok: loaded, stale: !loaded || !!lastError, error: lastError, commandsFromApi: served.commands, workspaceName };
    },
  };
}
